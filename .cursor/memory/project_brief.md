...
- **Offline Python sim** (optional): `mcts_train/` mirrors core rules for MCTS/RL experiments; does not ship with the game. **Mctsland**: nested history (attack/spree/deploy/fortify); ephemeral MCTS for attack/spree only; one-shot bandit deploy/fortify; **calibration ablation** via ``--mcts-decisions``.