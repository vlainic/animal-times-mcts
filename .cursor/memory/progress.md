...
- **Python `mcts_train`**: Offline Milos simulator + **Chaotic** / **Rookie** / **Mctsland**; **not** shipped in export.
  ...
  - **ChaoticBotPlayer** (`players/chaotic_bot_player.py`): ``--bots`` type **`0``**. Skip REINFORCE/FORTIFY; one random Combat + overrun bulk slide; random DeployPlace. AoD 1-unit attacks via ``legal_actions``. Wired in ``smoke_rollout._make_bot`` / ``parse_bots_spec`` (``012``).
...
- **Chaotic**: all-Chaotic seats (``--bots 0000``) often stall at outer ``max_steps`` (random play rarely eliminates); mixed patterns (e.g. ``0122``) finish normally.
