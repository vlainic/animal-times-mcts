...
- Constraints:
  ...
  - Optional **Python** rollout sim (`Python/mcts_train/`) is for offline tooling only; shipped game stays GDScript-only. Python sim uses fresh entropy per ``new_game`` (no baked-in replay unless you add seeds later).