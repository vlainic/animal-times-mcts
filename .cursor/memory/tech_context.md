...
- **Optional Python** (dev / training only, not in build): `mcts_train/` at repo root; scripts in `scripts/` via ``_bootstrap.setup()``. **`mcts_search`**: ``run_mcts_attack``, ``run_mcts_spree``, ``run_mcts_placement`` — ephemeral MCTS (defaults: 100 iterations, depth 5, breadth 5). **`MctslandBotPlayer`**: placement **session cache**; fortify strip-then-place; ATTACK + spree (full-attack required). **History**: nested JSON in ``data/``; ``ensure_history_bundle`` + fixed worker merge. **Training**: **`mcts_selfplay.py`** (default ``--full-attack``); **eval**: **`mcts_calibrate.py`**; **smoke**: **`smoke_rollout.py``** + failure dumps. **``rollout_limits.py``**: ``micro_step_cap``. **Parallelism**: ``--workers``. **Inference**: ``--mcts-history PATH``, ``from_history_file``, ``history_readonly=True``. ``data/``, ``logs/``, ``__pycache__/`` gitignored.
