...
- **Optional Python** (dev / training only, not in build): `mcts_train/` at repo root; scripts in `scripts/` via ``_bootstrap.setup()``. **Bots** (``--bots`` pattern): **`0`=`ChaoticBotPlayer`**, **`1`=`RookieBotPlayer`**, **`2`=`MctslandBotPlayer`**. **Chaotic**: skip reinforce/fortify; one random Combat + overrun slide; random deploy. **`mcts_search`**: ``run_mcts_attack``, ``run_mcts_spree`` — ephemeral MCTS (defaults: 100 iterations, depth 5, breadth 5). **`MctslandBotPlayer`**: one-shot DEPLOY/FORTIFY bandit distribute (default softmax); deploy 2-tuple + fortify 6-tuple history keys; ATTACK + spree (full-attack required); **`mcts_decisions`** frozenset for per-type Rookie fallback (calibration ablation). **History**: nested ``attack``/``spree``/``deploy``/``fortify`` in ``data/``; ``ensure_history_bundle`` + worker merge. **Training**: **`mcts_selfplay.py`**; **eval**: **`mcts_calibrate.py`** (checkpoint JSON, ``--mcts-decisions`` ablation); **smoke**: **`smoke_rollout.py`** (plumbs ``mcts_decisions`` kwarg, no CLI). **``rollout_limits.py``**: ``micro_step_cap`` + random legal fallback. **Parallelism**: ``--workers``, ``--batch-size``. **Placement CLI**: ``--placement-distribute``, ``--placement-softmax-temp``. **Inference**: ``--mcts-history PATH``, ``from_history_file``, ``history_readonly=True``.
...
- Files of Interest:
  ...
  - `Players/Chaotic/chaotic_bot_player.gd`: Godot chaotic bot (reference); Python port `mcts_train/players/chaotic_bot_player.py`
